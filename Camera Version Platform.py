...
class Window():
    def __init__(self):
        ...
        self.capture1 = pylon.InstantCamera()
        self.EO = False
    ...
    def Start_Detection(self):
        ...
        # try:
        # try:
        self.capture1 = pylon.InstantCamera(pylon.TlFactory.GetInstance().CreateFirstDevice())
        # 开始读取图像
        self.capture1.StartGrabbing(pylon.GrabStrategy_LatestImageOnly)
        converter = pylon.ImageFormatConverter()
        # 转换为OpenCV的BGR彩色格式
        converter.OutputPixelFormat = pylon.PixelType_BGR8packed
        converter.OutputBitAlignment = pylon.OutputBitAlignment_MsbAligned
        while self.capture1.IsGrabbing():
            t1 = time.time()
            fps = 0.0
            grabResult = self.capture1.RetrieveResult(1000, pylon.TimeoutHandling_ThrowException)
 ...
 chk_index - 1) + len(
                            ...
                            if immobile(bbox, previous_bbox_co_str) == True:
                                ...
                                if (str((class_name + str(t))) in str(cache_matrix)) and (
                                    ...
                                    cv2.putText(frame,
                                                class_name + "-" + str(track.track_id) + "waited_time : " + str(
                                                    t_spending),
                    ...
                    cv2.rectangle(frame, (int(bbox[0]), int(bbox[1] - 30)),
                                  ...
                                  color,
                                  -1)
                    cv2.putText(frame, class_name + "-" + str(track.track_id), (int(bbox[0]), int(bbox[3] + 20)), 0,
                                0.75,
                                (255, 255, 255), 2)
